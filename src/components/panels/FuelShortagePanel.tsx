...
function renderRow(s: FuelShortageEntry): string {
...
function buildHtml(
  data: ListFuelShortagesResponse,
  ...
  const confirmed = data.shortages.filter(s => s.severity === 'confirmed').length;
  const watch = data.shortages.filter(s => s.severity === 'watch').length;
  ...
  const attribution = attributionFooterHtml({
    ...
    sampleSize: data.shortages.length,
  ...
  const drawer = selectedId ? renderDrawer(detailLoading, detail) : '';
...
export function FuelShortagesPanelContent() {
  const [data, setData] = useState<ListFuelShortagesResponse | null>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [detail, setDetail] = useState<GetFuelShortageDetailResponse | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [loadError, setLoadError] = useState<string | null>(null);
  const unmountedRef = useRef(false);
  const selectedIdRef = useRef<string | null>(null);

  const loadDetail = useCallback(async (shortageId: string) => {
    setSelectedId(shortageId);
    selectedIdRef.current = shortageId;
    setDetailLoading(true);
    try {
      const d = await getSupplyChainClient().getFuelShortageDetail({ shortageId });
      if (unmountedRef.current || selectedIdRef.current !== shortageId) return;
      setDetail(d);
    ...
    try {
      ...
      if (hydrated) {
        ...
        void getSupplyChainClient().listFuelShortages({ country: '', product: '', severity: '' }).then(live => {
          if (unmountedRef.current || !live?.shortages?.length) return;
          setData(live);
      ...
      if (live.upstreamUnavailable || !live.shortages?.length) {
        setLoadError('Fuel shortage registry unavailable');
        return;
      }
      setData(live);
      const shortagesRecord: Record<string, FuelShortageEntry> =
      ...
      if (unmountedRef.current) return;
      setLoadError('Fuel shortage registry error');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('energy:open-fuel-shortage-detail', handler);
    return () => window.removeEventListener('energy:open-fuel-shortage-detail', handler);
  }, [loadDetail]);
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (row?.dataset.shortageId) {
      void loadDetail(row.dataset.shortageId);
    }
  ...
  return (
    <div
      dangerouslySetInnerHTML={{ __html: buildHtml(data!, selectedId, detailLoading, detail) }}
      onClick={handleClick}
    />
  );
}

export function FuelShortagesPanel() {
  return (
    <PanelShell
      id="fuel-shortages"
      title="Global Fuel Shortage Registry"
      infoTooltip="Global fuel-shortage alert registry (petrol, diesel, jet, heating oil). Severity (confirmed / watch) is a classifier output, not a client derivation. Every row carries the full evidence source list — see /docs/methodology/shortages for the threshold spec + classifier version."
      defaultRowSpan={2}
    >
      <FuelShortagesPanelContent />
    </PanelShell>
  );
}
