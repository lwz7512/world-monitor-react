...
import { fetchOtherTokens } from '@/services/market';
...
async function fetcher(_signal: AbortSignal): Promise<TokenData[]> {
  return fetchOtherTokens();
}

export function OtherTokensPanelContent() {
  const { data, loading, error, refetch } = usePanelData(fetcher, {
    hydrationKey: 'otherTokens',
    ttlMs: 5 * 60 * 1000,
  });
...
export function OtherTokensPanel() {
  return (
    <PanelShell
      id="other-tokens"
      title="Alt Tokens"
